...
if (isset($_GET['id']) && is_numeric($_GET['id']) && $_GET['id'] > 0) {
    ...
    $qry = $conn->query("SELECT * from `categories` where id = $id_int ");
    if ($qry->num_rows > 0) {
...
// else {
//     $_settings->set_flashdata('error', 'Category ID provided is Unknown');
...
<div class="card card-outline card-info">
    <div class="card-header">
        <h3 class="card-title">
            <?php echo isset($id) ? "Cập nhật " : "Tạo " ?> Danh mục
        </h3>
    </div>
    <div class="card-body">
        <form action="" id="category-form">
            ...
            <div class="form-group">
                ...
                <select name="status" id="status" class="custom-select selevt">
                    ...
                    <option value="0" <?php echo isset($status) && $status == 0 ? 'selected' : '' ?>>Không sẵn sàng
                    </option>
                </select>
            </div>
...
$(document).ready(function() {
    $('#category-form').submit(funct ion(e) {
        e.preventDefault();
        var _this = $(this)
        $('.err-msg').remove();
        start_loader();
