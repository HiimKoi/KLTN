								...
								<td align="center">
									...
									<div class="dropdown-menu" role="menu">
										<a class="dropdown-item"
											href="?page=orders/view_order&id=<?php echo $row['id'] ?>">Xem đơn hàng</a>
										<?php if ($row['paid'] == 0 && $row['status'] != 4): ?>
											<a class="dropdown-item pay_order" href="javascript:void(0)"
												data-id="<?php echo $row['id'] ?>">Đã thanh toán</a>
										<?php endif; ?>
...
<script>
	$(document).ready(function () {
		$('.delete_data').click(function () {
			_conf("Bạn có chắc chắn xóa đơn hàng này vĩnh viễn không?", "delete_order", [$(this).attr(
				'data-id')])
		})
		$('.pay_order').click(function () {
			_conf("Bạn có chắc chắn đánh dấu đơn đặt hàng này là đã thanh toán không?", "pay_order", [$(
				this).attr('data-id')])
		})
		$('table').dataTable({
			language: {
