...
if (isset($_GET['id']) && is_numeric($_GET['id']) && $_GET['id'] > 0) {
	...
	$qry = $conn->query("SELECT * from `inventory` where id = '$id_int' ");
	if ($qry->num_rows > 0) {
...
<div class="card card-outline card-info">
	<div class="card-header">
		<h3 class="card-title">
			<?php echo isset($id) ? "Update " : "Tạo mới " ?> Sản phẩm
		</h3>
	</div>
	<div class="card-body">
		<form action="" id="inventory-form">
			...
			<div class="form-group">
				...
				<input type="number" class="form-control form" required name="quantity"
					value="<?php echo isset($quantity) ? $quantity : '' ?>">
			</div>
			<div class="form-group">
				<label for="price" class="control-label">Giá</label>
				<input type="number" step="any" class="form-control form" required name="price"
