...
if (isset($_GET['c']) && isset($_GET['s'])) {
    $c_id = $_GET['c'];
    $s_id = $_GET['s'];
    $sanitized_c_id = filterHashMd5($c_id);
    $sanitized_sub_id = filterHashMd5($s_id);

    $cat_qry = $conn->query("SELECT * FROM categories where md5(id) = '{$sanitized_c_id}'");
    if ($cat_qry->num_rows > 0) {
        $result = $cat_qry->fetch_assoc();
        $title = $result['category'];
        $cat_description = $result['description'];
    }
    $sub_cat_qry = $conn->query("SELECT * FROM sub_categories where md5(id) = '{$sanitized_sub_id}'");
                ...
                <div class="tab-content pt-2">
                    <div class="tab-pane fade show active" id="book">
                        ...
                        <div class="row gx-2 gx-lg-2 row-cols-1 row-cols-md-3 row-cols-xl-3">
                            ...
                            $whereData = "";
                            if (isset($_GET['search'])) {
                                $condition = escape_string($_GET['search']);
                                // $whereData = " and (title LIKE '%{$_GET['search']}%' or author LIKE '%{$_GET['search']}%' or description LIKE '%{$_GET['search']}%')";
                                $whereData = " and (title LIKE '%{$condition}%' or author LIKE '%{$condition}%' or description LIKE '%{$condition}%')";

                            } elseif (isset($_GET['c']) && isset($_GET['s'])) {
                                $whereData = " and (md5(category_id) = '{$sanitized_c_id}' and md5(sub_category_id) = '{$sanitized_sub_id}')";
                            } elseif (isset($_GET['c']) && !isset($_GET['s'])) {
                                // $whereData = " and md5(category_id) = '{$_GET['c']}' ";
                                $whereData = " and md5(category_id) = '{$sanitized_c_id}' ";
                            } elseif (isset($_GET['s']) && !isset($_GET['c'])) {
                                // $whereData = " and md5(sub_category_id) = '{$_GET['s']}' ";
                                $whereData = " and md5(sub_category_id) = '{$sanitized_sub_id}' ";
                            }

                            // echo "SELECT * FROM `products` where status = 1 {$whereData} order by rand() ";
